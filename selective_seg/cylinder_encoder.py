...
class Cylinder_VFE(nn.Module):
    ...
    def __init__(self,
                 ...
                 model_mode='voxel',
                 compress_dim=None):
        super(Cylinder_VFE, self).__init__()
        ...
        if model_mode == 'voxel':
            ...
            self.feat_model = nn.Sequential(
                ...
                build_conv_layer(conv_cfg, in_channels, 64, 1, bias=False),
                build_norm_layer(norm_cfg, 64)[1],
                nn.ReLU(),

                build_conv_layer(conv_cfg, 64, 128, 1, bias=False),
                build_norm_layer(norm_cfg, 128)[1],
                nn.ReLU(),

                build_conv_layer(conv_cfg, 128, 256, 1, bias=False),
                build_norm_layer(norm_cfg, 256)[1],
                nn.ReLU(),

                build_conv_layer(conv_cfg, 256, 256, 1)
            )
            '''
            ...
            self.feat_model = nn.Sequential(
                ...
                build_conv_layer(conv_cfg, 64, 64, 1, bias=False),
                build_norm_layer(norm_cfg, 64)[1],
                nn.ReLU(),
                ...
                build_conv_layer(conv_cfg, 64, 64, 1)
            )
        else:
            self.feat_model = nn.Sequential(
        ...
        if compress_dim is not None:
            '''
            self.feat_compression = nn.Sequential(
    ...
    def forward(self,
        ...
        # trick for sync-bn 1D
        features = features.t().unsqueeze(0)

        point_feats = self.feat_model(features)

        point_feats = point_feats.squeeze(0).t()
        #voxel_feats, voxel_coors = self.vfe_scatter(point_feats, coors)

        # unique xy grid index
        unq, unq_inv, unq_cnt = torch.unique(coors, return_inverse=True, return_counts=True, dim=0)
        voxel_coors = unq

        voxel_feats = torch_scatter.scatter_max(point_feats, unq_inv, dim=0)[0]

        if self.compress_dim is not None:
            voxel_feats = self.feat_compression(voxel_feats)
